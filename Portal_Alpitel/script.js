...
const session = JSON.parse(localStorage.getItem('alpitel_session'));
...
function applyPermissions() {
    const { role } = session;
    const authorizedRoles = ['supervisor', 'admin', 'gestao', 'frota'];
    
    if (authorizedRoles.includes(role)) {
        if (elements.maintSection) elements.maintSection.classList.remove('hidden');
    } else {
        if (elements.maintSection) elements.maintSection.classList.add('hidden');
    }
}
...
function renderFleetTable() {
    ...
    fleet.forEach(veiculo => {
        ...
        tr.innerHTML = `
            <td data-label="Veículo"><strong>${veiculo.modelo}</strong></td>
            <td data-label="Placa">${veiculo.placa}</td>
            <td data-label="Status"><span class="badge ${veiculo.status}">${veiculo.status.replace(/[-_]/g, ' ')}</span></td>
            <td data-label="KM">${veiculo.km.toLocaleString()} km</td>
            <td data-label="Ação">
...
function renderAllVehicles() {
    ...
    sortedFleet.forEach(v => {
        ...
        row.innerHTML = `
            <td data-label="Modelo" style="padding: 12px; border-bottom: 1px solid #f1f5f9;">${v.modelo}</td>
            <td data-label="Placa" style="padding: 12px; border-bottom: 1px solid #f1f5f9;">${v.placa}</td>
            <td data-label="KM" style="padding: 12px; border-bottom: 1px solid #f1f5f9;">${v.km} km</td>
            <td data-label="Status" style="padding: 12px; border-bottom: 1px solid #f1f5f9;"><span class="badge ${v.status.replace('_', '-')}">${v.status.charAt(0).toUpperCase() + v.status.slice(1).replace('_', ' ')}</span></td>
        `;
        elements.allVehiclesTable.appendChild(row);
    });
}
