...
function applyPermissions() {
    ...
    if (role === 'colaborador' || role === 'manutencao' || role === 'supervisor' || role === 'admin' || role === 'gestao' || role === 'frota') {
        elements.maintSection.classList.remove('hidden');
    }
}

function updateDashboardStats() {
    elements.stats.total.textContent = fleet.length;
    elements.stats.disponiveis.textContent = fleet.filter(v => v.status === 'disponivel').length;
    elements.stats.manutencao.textContent = fleet.filter(v => v.status === 'manutencao' || v.status === 'aguardando_oficina' || v.status === 'reparo').length;
}
...
window.openHistory = function(placa) {
    ...
    elements.modalTitle.textContent = `${v.modelo} (${v.placa})`;
...
document.getElementById('btn-logout').addEventListener('click', () => {
    ...
    window.location.href = '../Portal Alpitel/index.html';
});
